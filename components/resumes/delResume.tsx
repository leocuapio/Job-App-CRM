...
export default function DeleteResume({open, handleClose, delResume, getAllResumes}: Params) {
    async function handleAgree() {
        const res = await fetch(`/api/resumes/${delResume.id}`,
            {
                method: "DELETE"
            }
        )
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description" sx={{ color: '#6d655c' }}>
            You are deleting {delResume.id}. Confirm or Deny below.
          </DialogContentText>
        </DialogContent>
